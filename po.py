...
class CryptoTradingEnv(gym.Env):  # gym.Env로 수정
    ...
    def step(self, action):
        ...
        self.truncated = self.current_step >= 100  # 100 스텝이 넘으면 종료되는 로직

        return self._next_observation(), reward, self.done, self.truncated, {}
...
def buy_crypto_currency(ticker, amount):
    ...
    if current_price is not None:
        print(f"Buying {amount} of {ticker} at {current_price} KRW")
        # 여기에 실제 매수 API 호출을 추가
        return True
    return False
...
def sell_crypto_currency(ticker, amount):
    current_price = pyupbit.get_current_price(ticker)
    if current_price is not None:
        print(f"Selling {amount} of {ticker} at {current_price} KRW")
        # 여기에 실제 매도 API 호출을 추가
        return True
    return False
